...
```
sudo tee $HOME/inery-testnet-faucet-tasks/$IneryAccname/pushTrans.js >/dev/null <<EOF
...
const account = "$IneryAccname";
const actor = "$IneryAccname";
...
const DestroyTrancsaction = async (id) => {
  try {
    ...
    console.log("Record destroyed by $IneryAccname\n\n");
